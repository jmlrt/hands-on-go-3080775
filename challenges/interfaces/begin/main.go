...
func (l letterCounter) count(text string) int {
	result := 0
...
func (n numberCounter) count(text string) int {
	result := 0
	for _, character := range text {
...
func (s symbolCounter) count(text string) int {
	result := 0
	for _, character := range text {
		if !unicode.IsNumber(character) && !unicode.IsLetter(character) {
			result++
		}
	}
	return result
